...
program test_outputlog_finalize
  ...
  use mom_outputlog_methods, only : get_ring_state, get_lstop_ring_state, check_file_completion
...
contains
  ...
  !> Drives get_ring_state/check_file_completion through one run (real,
  !! advancing clock -- unlike test_outputlog_freqn.F90, this file genuinely
  !! needs one, since get_lstop_ring_state's prevRingTime only becomes
  !! meaningful through real ESMF ring-detection), then replays
  !! mom_cap.F90's real two-call finalize sequence (plain check_file_completion,
  !! then get_lstop_ring_state+check_file_completion) at the final clock
  !! state, asserting each call catches the file only it can.
  subroutine run_lstop_case(freq, start_hour, run_hours, is_passing)
    ...
    type(ESMF_Time)          :: startTime, stopTime, currTime, refTime, lastrestart, prevring
    ...
    integer :: ierr, rc, count
    logical :: ringing, filecomplete
    ...
    ! Fixture files (*.nc) AND log_restart_fh's real output (this file's
    ! whole point is verifying real log content, unlike
    ! test_outputlog_freqn.F90 -- format YYYYMMDD.HHMMSS.mom6.<chour> or
    ! .mom6.lstop.<chour>, no .nc extension) from a prior case must not
    ! leak into this one.
    ...
    ! --- Main loop. CONFIRMED-CORRECT ring detection (see
    ! test_outputlog_freqn.F90's own extensive derivation, empirically
    ! verified against real PET-log output): reset count=0 EVERY iteration
    ! before advancing (ringingAlarmCount is NOT auto-reset by ESMF on
    ! ticks where nothing rings); advance FIRST; use ringingAlarmCount from
    ! THAT SAME advance call; read post-advance currTime, which IS
    ! numerically the same instant production's own nextTime represents at
    ! the point outputlog_run reads it. get_ring_state handles turning the
    ! alarm off internally -- no separate AlarmRingerOff call needed here.
    ! Every ring's fixture is created (schema+padding, incomplete) but
    ! NEVER completed in-loop -- "15" is deliberately left pending until
    ! after the loop, matching the real FMS-lag scenario this test exists
    ! to check. No logging here either (check_file_completion called
    ! without logtime/complog) -- only the two finalize calls below matter
    ! for this test's log-content assertions.
    do while (.not. ESMF_ClockIsStopTime(clock, rc=ierr))
       ...
       if (ringing) then
          timestr = get_timestr(currTime - cf_n%filename_fhoffset, rc=ierr)
          ...
          call get_ring_state(currTime, cf_n%alarm, cf_n, state_n, comm, isroot, rootpe, outputdir, rc)
          call esmf_err(rc, subname, "get_ring_state")
       end if

       call check_file_completion(state_n, comm, isroot, rootpe, startTime, lastrestart, &
            filecomplete, rc)
       call esmf_err(rc, subname, "check_file_completion (main loop)")
    end do
    ...
    ! first, then the lstop path -- same clock state (currTime is still
    ! stopTime, unchanged since the loop ended), no advance between them.
    ! PLAIN: no new ring here -- just check_file_completion on whatever's
    ! already being tracked ("15"), now with a real logtime/complog so the
    ! log actually gets written this time.
    call check_file_completion(state_n, comm, isroot, rootpe, startTime, lastrestart, &
         filecomplete, rc, logtime=currTime-cf_n%logname_fhoffset, complog='mom6.'//trim(chour))
    call esmf_err(rc, subname, "check_file_completion (finalize, plain)")
    is_passing = filecomplete   ! plain call must catch "15"

    ! LSTOP: matches production's real two-step dispatch -- get_lstop_ring_state
    ! sets up tracking for "21" (already knows to find the fixture created
    ! above, since prevRingTime hasn't changed since it was read earlier),
    ! then check_file_completion resolves it, this time with the
    ! lstop-specific logtime/complog.
    call get_lstop_ring_state(cf_n%alarm, tincrement, cf_n, state_n, comm, isroot, rootpe, &
         outputdir, prevring, rc)
    call esmf_err(rc, subname, "get_lstop_ring_state (finalize)")

    call check_file_completion(state_n, comm, isroot, rootpe, startTime, lastrestart, &
         filecomplete, rc, logtime=prevring, complog='mom6.lstop.'//trim(chour))
    call esmf_err(rc, subname, "check_file_completion (finalize, lstop)")
    is_passing = is_passing .and. filecomplete   ! lstop call must catch "21"
